...
- Emits translated Opus frames (`onAudioFrame(tag, chunk, timestamp, payload)`) and the translated (target-language) transcript (`onTranscription(transcript, targetLanguage, isInterim)`) — transcript **deltas** are `isInterim: true`. The **final** comes from the transcript-done event on the general `/v1/realtime` endpoint; the `/v1/realtime/translations` endpoint sends no transcript-done, so the last interim is re-emitted as the final at the talk-stop silence boundary (`finalizePendingTranscript`) and on close. Both suppressed when `emitTranscripts === false`
